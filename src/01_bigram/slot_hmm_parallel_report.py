"""
slot_hmm_parallel_report.py
===========================
並列化版スロット文法HMM学習スクリプト（レポート出力付き）

- joblib によるマルチプロセッシングで多重スタート (seed) を並列化
- Full HMM と Left-to-Right HMM の両方を学習
- 学習後に遷移確率/放射確率のヒートマップ、BIC比較、テキストレポートを出力
"""
...
def encode_words(words, char2idx):
    ...
    return np.ascontiguousarray(X), lengths

# ── 並列用学習関数 ────────────────────────────────────────────────────
# これらはマルチプロセスマップの対象になるためトップレベルに配置します
...
def train_ltr_seed(seed, k, X, lengths, ltr_iter=20):
    try:
        model = HMM_CLASS(
            n_components=k,
            n_iter=1,
...
# ── 並列学習ラッパー ───────────────────────────────────────────────────
def fit_hmm_parallel(k, X, lengths, mode="Full"):
    if mode == "Full":
        ...
        results = Parallel(n_jobs=N_JOBS)(
            delayed(train_ltr_seed)(seed, k, X, lengths, LTR_ITER)
            for seed in range(N_RESTARTS)
        )
...
# ── 計算と表示ユーティリティ ───────────────────────────────────────────
def compute_bic(model, X, lengths, k, V):
...
def plot_emission(model, k, topology, all_chars, out_path):
    state_labels = [_state_label(i, k) for i in range(k)]
...
# ════════════════════════════════════════════════════════════════════════════
# メインプロセス（Windowsでのマルチプロセス安全性のためのブロック）
# ════════════════════════════════════════════════════════════════════════════
if __name__ == "__main__":
    try:
        if HMM_CLASS.__name__ == 'CategoricalHMM':
            print("  [hmm] Using CategoricalHMM")
        else:
            print("  [hmm] Using MultinomialHMM (fallback)")
        
        print(f"  [parallel] Using {N_JOBS} cores for training")
        ...
        for k in K_RANGE:
            ...
            # ── Full HMM
            print(f"  [Full] 学習中... (N_RESTARTS={N_RESTARTS})")
            model_full, ll_full_max = fit_hmm_parallel(k, X_all, L_all, mode="Full")

            if model_full is None:
                print(f"  [Full] k={k}: 学習失敗")
                continue

            bic_full, aic_full, ll_full = compute_bic(model_full, X_all, L_all, k, V)
            plot_transition(model_full, k, "Full", OUT_DIR / f"transition_full_k{k}.png")
            plot_emission(model_full, k, "Full", all_chars, OUT_DIR / f"emission_full_k{k}.png")

            if k == 3:
                best_k3_full = model_full

            # ── Left-to-Right HMM
            print(f"  [L-to-R] 学習中... (N_RESTARTS={N_RESTARTS}, LTR_ITER={LTR_ITER})")
            model_ltr, ll_ltr_max = fit_hmm_parallel(k, X_all, L_all, mode="LTR")

            if model_ltr is None:
                print(f"  [L-to-R] k={k}: 学習失敗")
                bic_ltr, aic_ltr, ll_ltr = np.nan, np.nan, np.nan
            else:
                bic_ltr, aic_ltr, ll_ltr = compute_bic(model_ltr, X_all, L_all, k, V)
                plot_transition(model_ltr, k, "L-to-R", OUT_DIR / f"transition_ltr_k{k}.png")
                plot_emission(model_ltr, k, "L-to-R", all_chars, OUT_DIR / f"emission_ltr_k{k}.png")
